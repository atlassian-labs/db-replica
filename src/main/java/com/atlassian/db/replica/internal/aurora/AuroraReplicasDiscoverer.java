...
public final class AuroraReplicasDiscoverer {
    ...
    private List<String> fetchReplicasServerIds(Connection connection) throws SQLException {
        ...
        final String sql = "SELECT server_id, durable_lsn, current_read_lsn, feedback_xmin, round(extract(milliseconds from (now()-last_update_timestamp))) as state_lag_msec, replica_lag_in_msec " +
        ...
        try (ResultSet rs =
            ...
            while (rs.next()) {
                ...
                logger.debug(String.format(
                    "server_id=%s, replica_lag_in_ms=%d, durable_lsn=%d, current_read_lsn=%d, feedback_xmin=%d",
                    serverId,
                    replicaLagInMs,
                    durableLsn,
                    currentReadLsn,
